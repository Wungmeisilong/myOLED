...
/* 垂直滚动特效函数 */
void OLED_ScrollImageDown(int16_t X, int16_t Y, uint8_t Width, uint8_t Height, 
                         const uint8_t *Image, uint16_t delay_ms)
{
    // 计算总页数（向上取整）
    uint8_t total_pages = (Height - 1) / 8 + 1;
    
    // 清空目标区域（只清一次避免闪烁）
    OLED_ClearArea(X, Y, Width, Height);
    
    // 存储原始显示缓存状态
    uint8_t buf_backup[8][128];
    memcpy(buf_backup, OLED_DisplayBuf, sizeof(OLED_DisplayBuf));
    
    // 每次增加1页（8行）
    for (uint8_t page = 0; page <= total_pages; page++)
    {
        // 恢复原始缓存状态（避免残留）
        memcpy(OLED_DisplayBuf, buf_backup, sizeof(OLED_DisplayBuf));
        
        // 仅显示当前页数的图像部分
        for (uint8_t j = 0; j < page; j++)
        {
            for (uint8_t i = 0; i < Width; i++)
            {
                if (X + i >= 0 && X + i <= 127) 
                {
                    int16_t abs_page = Y / 8;
                    int16_t shift = Y % 8;
                    ...
                    if (abs_page + j >= 0 && abs_page + j <= 7)
                    {
                        // 写入当前页
                        OLED_DisplayBuf[abs_page + j][X + i] |= 
                            Image[j * Width + i] << shift;
                    }
                    
                    if (j < page - 1 && abs_page + j + 1 >= 0 && abs_page + j + 1 <= 7)
                    {                    
                        // 写入下一页
                        OLED_DisplayBuf[abs_page + j + 1][X + i] |= 
        ...
        if (page == total_pages)
        {
            ...
            if (partial_bits)
            {
                for (uint8_t i = 0; i < Width; i++)
                {
                    if (X + i >= 0 && X + i <= 127) 
                    {
                        int16_t abs_page = Y / 8 + total_pages - 1;
                        int16_t shift = Y % 8;
                        if (Y < 0) {
                            abs_page -= 1;
                            shift += 8;
                        ...
                        if (abs_page >= 0 && abs_page <= 7)
                        {
                            // 屏蔽超出高度的部分
                            uint8_t mask = (1 << partial_bits) - 1;
                            OLED_DisplayBuf[abs_page][X + i] |= 
                                (Image[(total_pages - 1) * Width + i] & mask) << shift;
                        }
                    }
                }
            }
